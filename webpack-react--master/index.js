...
http
  .createServer(async (req, res) => {
    ...
    if (req.method === "POST" && req.url === "/") {
      ...
      // 拉取仓库最新代码
      execSync(
        `git clone https://github.com/quechenping/${data.repository.name}.git ${projectDir}`,
        {
          stdio: "inherit",
        }
      );
    ...
    // 创建 docker 容器
    execSync(
      `docker run -d -p 8080:80 --name ${data.repository.name}-container  ${data.repository.name}-image:latest`,
      {
        stdio: "inherit",
      }
    );
